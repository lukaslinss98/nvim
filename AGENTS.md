...
- `treesitter.lua` — nvim-treesitter on the `main` branch (`master` is EOL and its query directives crash on Neovim 0.12); requires the `tree-sitter` CLI (`brew install tree-sitter-cli`) to build parsers, and `lazy = false` since `main` does not support lazy-loading. 19 parsers (lua, js/ts/tsx, python, go, html, css, bash, java, kotlin, markdown, markdown_inline, turtle, yaml, terraform, hcl, rust, helm) installed asynchronously into `stdpath("data")/site`. `main` has no `ensure_installed`/`highlight`/`indent` options: a `FileType` autocommand calls `vim.treesitter.start` and sets the treesitter `indentexpr`, but only for languages that ship an `indents` query (helm/kotlin/markdown_inline ship none, and their indentation would otherwise flatten to column 0). Maps `*/charts/templates/*.yaml` to the `helm` filetype
